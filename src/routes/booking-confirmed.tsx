  ...
  return (
    <div className="px-6 py-24">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-primary/30 bg-card p-10 text-center">
        ...
        <h1 className="mt-6 font-display text-4xl text-cream">Booking Request Received</h1>
        <p className="mt-3 text-sm text-muted-foreground">
          Thank you{data?.customer?.name ? `, ${data.customer.name}` : ""}! Our team will contact you
          shortly with a detailed quotation.
        </p>
        {data && (
        ...
        {data && (
          <div className="mt-8 rounded-3xl border border-primary/20 p-6 text-left text-sm text-muted-foreground">
            <p><span className="text-cream">Event:</span> {data.eventType}</p>
            <p><span className="text-cream">Date:</span> {data.details?.date || "—"}</p>
            <p><span className="text-cream">Guests:</span> {data.details?.guests || "—"}</p>
            <p className="mt-2">
              <span className="text-cream">Menu:</span>{" "}
              {data.pkg ? `${data.pkg} package — ` : ""}
              {data.dishes?.map((d) => d.name).join(", ") || "—"}
            </p>
            {data.requests?.length > 0 && (
              <p className="mt-2">
                <span className="text-cream">Requested dishes:</span>{" "}
                {data.requests.map((r) => r.name).join(", ")}
              </p>
            )}
            {data.addons?.length > 0 && (
              <p className="mt-2">
                <span className="text-cream">Additional services:</span> {data.addons.join(", ")}
              </p>
            )}
          </div>
        )}
        ...
        <div className="mt-9 flex flex-wrap justify-center gap-4 print:hidden">
          ...
          >
            <Download className="size-4" /> Download Summary (PDF)
          </button>
        </div>
      </div>
    </div>
